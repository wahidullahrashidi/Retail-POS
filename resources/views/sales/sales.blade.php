...
@push('styles')
    <link
        href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,500;0,600;1,400&family=Outfit:wght@300;400;500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap"
        rel="stylesheet">
    <style>
        /* ══════════════════════════════════════
                                           TOKENS
        ...
        .sl-table tbody tr:hover .row-acts {
            opacity: 1;
        }
    ...
    <div class="sl" x-data="salesPage()" x-init="init()">
        ...
        <div class="stat-strip">
            <div class="stat-tile" style="--ac:var(--blue)">
                <div class="st-label">Today's Revenue <span style="color:var(--blue)"><i class="fas fa-coins"></i></span>
                </div>
            ...
            <div class="stat-tile" style="--ac:var(--green)">
                <div class="st-label">Cash Sales <span style="color:var(--green)"><i class="fas fa-money-bill"></i></span>
                </div>
            ...
            <div class="stat-tile" style="--ac:var(--amber)">
                <div class="st-label">Loan Sales <span style="color:var(--amber)"><i class="fas fa-file-invoice"></i></span>
                </div>
            ...
            <div class="stat-tile" style="--ac:var(--red)">
                <div class="st-label">Refunded <span style="color:var(--red)"><i class="fas fa-rotate-left"></i></span>
                </div>
            ...
            <div class="stat-tile" style="--ac:var(--violet)">
                <div class="st-label">Avg Ticket <span style="color:var(--violet)"><i class="fas fa-receipt"></i></span>
                </div>
        ...
        <div class="sl-toolbar">
            <div class="search-box">
                ...
                <input class="sl-search" type="text" x-model="search" @input.debounce.350ms="loadSales()"
        ...
        <div class="sl-main" :class="selected ? 'panel-open' : ''" style="align-items:start">
                    ...
                    <table class="sl-table" x-show="sales.length > 0">
                        <thead>
                            <tr>
                                ...
                                <th>Cashier</th>
                                <th>Method</th>
                                <th @click="sort('total_amount')" class="cell-right">Total <i class="fas fa-sort"></i>
                                </th>
                                    ...
                                    <td @click.stop>
                                        <div class="row-acts">
                                            ...
                                            <button type="button" class="btn btn-ghost btn-sm"
                                                @click="reprintReceipt(s)" title="Reprint">
                                            ...
                                            {{-- <button type="button" class="btn btn-danger btn-sm" @click="deleteSale(s)"
                                                title="Delete">
            ...
            <div class="detail-panel" x-show="selected" x-cloak>
                ...
                <div class="dp-body">
                    ...
                    {{-- Items --}}
                    <div class="dp-section">
                        <div class="dp-section-title"><i class="fas fa-list"></i> Items (<span
                            ...
                            <template x-for="item in detailItems" :key="item.id">
                                <div class="item-row" :class="item.is_returned ? 'ir-returned' : ''">
                                    <div>
                                        ...
                                        <div class="ir-sku" x-text="item.sku + (item.is_returned ? ' · Returned' : '')">
                                        </div>
                                    </div>
                                    <div class="ir-right">
                                        <div class="ir-qty" x-text="item.quantity + ' × Af ' + fmt(item.unit_price)">
                                        </div>
                    ...
                    <div class="dp-section">
                        ...
                        <div class="tot-row"><span>Subtotal</span><span class="tot-mono"
                                x-text="'Af ' + fmt(selected?.subtotal||0)"></span></div>
                        <div class="tot-row" x-show="(selected?.discount_amount||0) > 0">
                            ...
                            <span class="tot-mono" style="color:var(--red)"
                                x-text="'- Af ' + fmt(selected?.discount_amount||0)"></span>
                        </div>
            ...
            <div class="modal-card">
                ...
                <div class="modal-body">
                        ...
                        <template x-for="item in refundItems.filter(i => i.max_refundable > 0)" :key="item.id">
                            <div class="refund-item" :class="item.selected ? 'selected' : ''"
                                ...
                                <div class="ri-info">
                                    ...
                                    <div class="ri-detail"
                                        x-text="item.sku + ' · ' + item.quantity + ' sold @ Af ' + fmt(item.unit_price)">
                                    </div>
                                </div>
                                <div @click.stop style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
                                    <span class="ri-amt" x-text="'- Af ' + fmt(calcRefundAmount(item))"></span>
                    ...
                    <div class="refund-summary" x-show="refundTotal > 0" x-cloak>
                        <div class="rs-row"><span>Items selected</span><span
                                x-text="refundItems.filter(i=>i.selected).length"></span></div>
                        <div class="rs-row"><span>Total refund amount</span><span
                                x-text="'Af ' + fmt(refundTotal)"></span></div>
                    </div>
            ...
            Alpine.data('salesPage', () => ({
                ...
                async openRefund(s) {
                    ...
                    try {
                        ...
                        if (!r.ok) {
                            throw new Error('Failed to load refund items.');
                        }
                ...
                async processRefund() {
                    ...
                    try {
                        ...
                        this.refundError = 'Network error.';
                    } finally {
                        this.saving = false;
                    }
                },
